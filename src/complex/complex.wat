(module
  ...
  (func $sqrt
    ...
    (if (f64.eq (local.get $imag) (f64.const 0))
      (then
        ;; Yup, so find the root of the real part and return it as either the real or imaginary part depending on its
        ;; sign
      ...
      (else
        ...
        (if (f64.eq (local.get $real) (f64.const 0))
          ...
          (else
            ;; Both parts are non-zero, so convert to polar coordinates, find the √mag, halve the angle, then convert
  ...
  (func $asinh
        ...
        (result f64 f64)
    (call $ln
      (call $add (local.get $a) (local.get $b) (call $sqrt_1pz2 (local.get $a) (local.get $b)))
    )
  )
    ...
    (call $mul
      ...
      (call $ln
        (call $add
          (call $sqrt
            (call $div
              (call $add (local.get $a) (local.get $b) (f64.const 1) (f64.const 0))
              (f64.const 2) (f64.const 0)
            )
          )
          (call $sqrt
            (call $div
              (call $sub (local.get $a) (local.get $b) (f64.const 1) (f64.const 0))
              (f64.const 2) (f64.const 0)
            )
          )
        )
  ...
  (func $atan
    ...
    (call $div
      (call $sub
        (call $ln
          (call $add (f64.const 1) (f64.const 0) (local.get $i_z_real) (local.get $i_z_imag))
        )
        (call $ln
          (call $sub (f64.const 1) (f64.const 0) (local.get $i_z_real) (local.get $i_z_imag))
        )
      )
  ...
  (func $atanh
    ...
    (call $div
      (call $sub
        (call $ln
          (call $add (f64.const 1) (f64.const 0) (local.get $a) (local.get $b))
        )
        (call $ln
          (call $sub (f64.const 1) (f64.const 0) (local.get $a) (local.get $b))
        )
      )
      (f64.const 2) (f64.const 0)
    )
